...
static BOOLEAN int_from_string(const char input[], int* output) {
    ...
    if (*end) {
        fprintf(stderr, "ErrorL %s is not a valid number.\n", input);
        return FALSE;
    }
    if (result < INT_MIN || result > INT_MAX) {
        fprintf(stderr, "Error: %ld is not in the range of an int. \n", result);
        return FALSE;
    }
    *output = result;
    return TRUE;
...
static BOOLEAN line_to_array(char inputline[], int array[], int* num_entered) {
    char* tok;
...
static BOOLEAN write_num(int array[], int array_count, int total,
                         const char* fname) {
    int count;
...
int main(int argc, char* argv[]) {
    ...
    write_num(nums, num_entered, sum, argv[FILE_ARG]);
    return EXIT_SUCCESS;
}
